...
inline void ImGui::FileBrowser::Display()
{
    ...
    else
    {
        ...
        // My Modification
        size_t rootSecCount = 0;
        bool firstButtonDrawn = false;
        for (auto& s : std::filesystem::current_path()) {
            rootSecCount++;
        }
...

    {
        ...
        for(unsigned int rscIndex = 0; rscIndex < fileRecords_.size(); ++rscIndex)
        {
            ...
            {
                if (std::filesystem::equivalent(this->GetDirectory(), std::filesystem::current_path()))
                {
                    continue;
                }
            }
